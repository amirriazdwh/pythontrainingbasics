...
def execbatchinitload(brow):
    ...
    try:
        ...
        logging.info('Session value of jobfailthreshold is %s, parallelism is %s',brow.batch_jobfail_threshold,brow.batch_parallelism)
        ...
        if failjobcnt != 0:
            logging.error('Batch Execution failed with success jobs count as %s and failed jobs count as %s',scsjobcnt,failjobcnt)
...
def create_batchrun_record(batchid,runtype,logpath,metadbconn):
    try:
        ...
        sqlquery = 'INSERT INTO {}.tblbatchrundetails (batch_id,run_category,run_starttime,run_status,run_logpath) VALUES({},{},getdate(),{},{})'\
                    .format(configurations.metadb_schema,batchid,"'"+runtype+"'","'"+configurations.runningstatus+"'","'" + logpath + "'")
...
def get_initjob_details(batchid,metadbconn):
    try:
        conn = metadbconn  
        cursor = conn.cursor()
        sqlquery = 'select * from {}.tbljobconfig where batch_id = {} order by load_priority'.format(configurations.metadb_schema,batchid)
        logging.info(sqlquery)
        cursor.execute(sqlquery)
        return cursor      
    except Exception as e:
...
def update_batchrun_record(batchrunid,status,scsjobcnt,failjobcnt,metadbconn):
    try:
        ...
        sqlquery = 'UPDATE {}.tblbatchrundetails \
                        ...
                        where batch_run_id = {}'.format(configurations.metadb_schema,"'"+status+"'",scsjobcnt,failjobcnt,batchrunid)
        logging.info(sqlquery)
        cursor.execute(sqlquery)
        conn.commit()
...
def executejob(q,brow,batchrunid):
    while True:
        ...
        try:
            ...
            logging.info('Job ID: %s, Thread ID & Name: %s & %s :: starting execution',jdatarow.job_id,threadid,threadname)
            jobrunid = create_jobrun_record(brow.batch_id,batchrunid,jdatarow.job_id,thmetadbconn)
            logging.info('Job ID: %s, Thread ID & Name: %s & %s :: created job run entry as %s',jdatarow.job_id,threadid,threadname,jobrunid)
            ...
            logging.info('Job ID: %s, Thread ID & Name: %s & %s :: sqoop log path is %s',jdatarow.job_id,threadid,threadname,sqplogpath)
            ...
            while j <= brow.batch_retry_count:
                try:
                    ...
                    logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: starting execution',jdatarow.job_id,threadid,threadname,j)            
                    prescriptstatus = ''
                    sqooploadstatus = ''
                    postscriptstatus = ''
                    ...
                    if jdatarow.pre_run_script != None:
                        try:
                            ...
                            if returncode  != 0:
                                ...
                                prescriptstatus = 'S'
                                logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: pre-script execution completed with output :',jdatarow.job_id,threadid,threadname,j)
                                logging.info(sout)
                        except Exception as e:
                            prescriptstatus = 'F'
                            logging.error('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: pre-script execution failed with error :',jdatarow.job_id,threadid,threadname,j)
                            logging.error(e)
                            raise Exception(e)                
                    else:
                        #Status None for pre script means that prescript was not applicable for this job
                        prescriptstatus = None
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: no pre-script found',jdatarow.job_id,threadid,threadname,j)

                    #Run the sqoop command for the job
                    try:
                        sqpcmd,sqpexeccmd = sqoop.gen_sqoop_command(jdatarow,brow,metadbconn,jobrunid,sqplogpath)
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: generated sqoop command is %s',jdatarow.job_id,threadid,threadname,j,sqpcmd.replace('....',' '))
                        ...
                        if sqooploadstatus == 'S':
                            logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: sqoop execution completed successfully',jdatarow.job_id,threadid,threadname,j)
                            
                            #Get the rows impacted by the sqoop command
                            reccnt = sqoop.get_records_count(sqplogpath)
                            logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: sqoop execution records retrived as %s',jdatarow.job_id,threadid,threadname,j,reccnt)
                    except Exception as e:
                    ...
                    if jdatarow.post_run_script != None:
                        try:
                            ...
                            if returncode  != 0:
                                ...
                                postscriptstatus = 'S'
                                logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: post-script execution completed with output :',jdatarow.job_id,threadid,threadname,j)
                                logging.info(sout)
                        except Exception as e:
                            postscriptstatus = 'F'
                            logging.error('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: post-script execution failed with error :',jdatarow.job_id,threadid,threadname,j)
                            logging.error(e)
                            raise Exception(e)                
                    else:
                        #Status None for pre script means that postscript was not applicable for this job
                        postscriptstatus = None
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: no post-script found',jdatarow.job_id,threadid,threadname,j)
                    
                    update_jobrun_record(jobrunid,configurations.successstatus,reccnt,thmetadbconn,sqplogpath,prescriptstatus,sqooploadstatus,postscriptstatus)
                    logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: updated job run id with status as success',jdatarow.job_id,threadid,threadname,j)
                    
                    #Updating the batch success job count 
                    with lock:
                        scsjobcnt += 1
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: updated success job count to %s',jdatarow.job_id,threadid,threadname,j,scsjobcnt)
                    break
                except Exception as e:
                    #Identifying the number of failed attempt and deriving the run needs to be done again
                    if j >= brow.batch_retry_count:                    
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: retries attempt exceeded. job failed',jdatarow.job_id,threadid,threadname,j)
                        logging.error(sys._getframe(0).f_code.co_name + ' : ' + __name__ + ' : Line ' + str(sys.exc_info()[-1].tb_lineno)  + ' : ' + str(e))

                        #Updating the job as failed as all retry attempts have failed
                        update_jobrun_record(jobrunid,configurations.failstatus,0,thmetadbconn,sqplogpath,prescriptstatus,sqooploadstatus,postscriptstatus)
                        #Updating the batch fail job count
                        with lock:
                            failjobcnt += 1
                        #Sending Job Failure email
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: sending job failure mail',jdatarow.job_id,threadid,threadname,j)
                        send_job_failemail(attachmentslist,brow.batch_name,jobname,e)
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: Mail sent',jdatarow.job_id,threadid,threadname,j)
                        raise Exception (e)
                    else:
                        logging.error(e)
                        logging.info('Job ID: %s, Thread ID & Name: %s & %s, Attempt No: %s :: execution failed. Initiating the next try after %s seconds',\
...
def create_jobrun_record(batchid,batchrunid,job_id,metadbconn):
    try:
        ...
        sqlquery = 'INSERT INTO {}.tbljobrundetails \
                            ...
                            VALUES \
                            ({},{},{},getdate(),{})'.format(configurations.metadb_schema,batchid,batchrunid,job_id,"'"+configurations.runningstatus+"'")
        logging.info(sqlquery)
        cursor.execute(sqlquery)
        conn.commit()
        cursor.execute('SELECT @@IDENTITY')
...
def update_jobrun_record(jobrunid,status,reccnt,metadbconn,sqplogpath,prescriptstatus,sqooploadstatus,postscriptstatus):
    try:
        conn = metadbconn  
        cursor = conn.cursor()
        ...
        sqlquery = 'UPDATE {}.tbljobrundetails \
                        ...
                        where job_run_id = {}'.format(configurations.metadb_schema,"'"+status+"'",reccnt,"'"+sqplogpath+"'", \
...
def get_failedjobs_logpaths(batchrunid,blogfilename):
    try:
        ...
        conn = metadbconn
        cursor = conn.cursor()
        sqlquery = 'Select job_logpath from {}.tbljobrundetails where batch_run_id = {} and run_status = {}'.format(configurations.metadb_schema, \
