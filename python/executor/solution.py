...
def get_last_month_data(cursor):
    data = cursor.execute(
        "select * from (select *, row_number() over(partition by Query_No order by Execution_date desc) "
        "as rnk from {} where month(Execution_date) = month(dateadd(m,-1, getdate()))) a where rnk = 1".format(
            conf.sql_result_table)).fetchall()
...
def get_last_day_data(cursor):
    prev_out = cursor.execute(
        "select Query_No,Output from (select *, row_number() over(partition by Query_No order by Execution_date desc) as rnk from {}"
        " where cast(Execution_date as date) = cast(dateadd(d,-1,getdate()) as date)) "
        "a where rnk = 1 ".format(conf.sql_result_table)).fetchall()
...
def insert_result(cursor, query_no, curr_cnt, variance, exec_dt, status, table_name):
    ...
    query = "insert into {0}(Query_No,Execution_date,Output,Variance_Per,status_time_count,table_name) values(?,?,?,?,?,?)".format(
        conf.sql_result_table)
...
def execute_queries(table_names, frequency):
    ...
    identifiers = "select distinct identifier from {}".format(
        conf.sql_config_table) if identifiers.upper() == "'ALL'" else identifiers
    freq_filter = "select distinct frequency from {}".format(
        conf.sql_config_table) if freq_filter.upper() == "'ALL'" else freq_filter
    ...
    test_query = "SELECT * from {0} where identifier in ({1}) and frequency in ({2})".format(conf.sql_config_table,
                                                                                             identifiers, freq_filter)

    print(test_query)

    cursor.execute(
        "SELECT * from {0} where identifier in ({1}) and frequency in ({2})".format(conf.sql_config_table, identifiers,
                                                                                    freq_filter))

    rows = cursor.fetchall()
    row_list = list(rows)
    job_id_list = ",".join(
        map(lambda x: "{}".format(int(getattr(x, 'Job_id'))), filter(lambda x: getattr(x, 'Job_id') != None, row_list)))
    ...
    data_to_write = spark.sql(
        "select cast(null as int) as Sr_No, cast(null as int) as Query_No, cast(null as String) as Table_Name, cast(null as String) as "
        "Query, cast(null as string) as Description, cast(null as int) as Expected_Result, cast(null as int) as Output, cast(null as date) as "
        "Table_Last_Refresh_Date, cast(null as string) as Table_Refresh_Frequency, cast(null as float) as Variance_As_Of_Last_Execution, cast(null as float) "
        "as Variance_As_Of_Last_Month, cast(null as timestamp) as Execution_Date_Time")
    f_count = 0
    ...
    if not rows:
        ...
        for row in rows:
            try:
                ...
                tbl_details = list(
                    filter(lambda x: getattr(x, 'job_id') == job_id, job_run_details)) if job_id else None
                ...
                last_month_variance = 0.0 if not last_month_cnt or not curnt_cnt else float(
                    curnt_cnt - last_month_cnt) / last_month_cnt
                ...
                tab_last_ref = tbl_last_run_date if not tab_last_ref or (
                        tbl_last_run_date and datetime.strptime(str(tbl_last_run_date),
                                                                '%Y-%m-%d') > datetime.strptime(tab_last_ref,
                                                                                                '%Y-%m-%d')) else tab_last_ref
                print("tab_last_ref = {}".format(tab_last_ref))

                data_to_write = data_to_write.unionAll(spark.sql(
                    """select cast({0} as int) as Sr_No, cast({1} as int) as Query_No, cast('{2}' as String) as Table_Name, cast("{11}" as string) as 
                    Query, cast("{3}" as string) as Description, cast({4} as int) as Expected_Result, cast({7} as int) as Output, cast('{5}' as date) as 
                    Table_Last_Refresh_Date, cast('{6}' as string) as Table_Refresh_Frequency, cast({8} as float) as Variance_As_Of_Last_Execution, 
                    cast({9} as float) as Variance_As_Of_Last_Month, cast('{10}' as timestamp) as Execution_Date_Time"""
                        .format(sr_no, query_no, query_re_split[0], desc, Expected_Count, tab_last_ref, freq, curnt_cnt,
                                format(variance * 100, '.3f'), format(last_month_variance * 100, '.3f'), exec_dt,
                                query)))
        ...
        final_data_to_write.dropna('all').coalesce(1).write.mode('overwrite').csv(
            "/warehouse/tablespace/external/hive/obtf_qc_output", header='true')
        subprocess.call('rm /home/cibg_uat_user/obtf/qc_framework/output/executor_result_{}.csv'.format(
            datetime.now().strftime('%Y%m%d')), shell=True)
        subprocess.call(
            'hadoop fs -copyToLocal /warehouse/tablespace/external/hive/obtf_qc_output/*.csv /home/'
            'cibg_uat_user/obtf/qc_framework/output/executor_result_{}.csv'.format(
                datetime.now().strftime('%Y%m%d')), shell=True)
        if f_count != 0:
            mail.send_Failuremail('/home/cibg_uat_user/obtf/qc_framework/output/executor_result_{}.csv'.format(
                datetime.now().strftime('%Y%m%d')))
        else:
            mail.send_mail('/home/cibg_uat_user/obtf/qc_framework/output/executor_result_{}.csv'.format(
                datetime.now().strftime('%Y%m%d')))
