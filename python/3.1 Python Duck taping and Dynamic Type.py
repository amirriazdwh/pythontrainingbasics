...
Let’s create a custom class that behaves like a sequence by implementing the necessary methods (__getitem__, __len__, and __contains__) without explicitly inheriting from the Sequence ABC.
...
AI-generated code. Review and use carefully. More info on FAQ.
Explanation:
MySequence Class: This class implements the methods __getitem__, __len__, and __contains__, which are required by the Sequence interface.
isinstance(my_seq, Sequence): This checks if my_seq is considered an instance of Sequence. The output is True because MySequence implements the necessary methods defined by Sequence.
Key Points:
Duck Typing: In Python, if an object implements the methods required by an interface, it is considered to conform to that interface, regardless of its inheritance hierarchy.
Dynamic Typing: Python’s dynamic nature allows you to create classes that implement specific methods without needing to inherit from a particular base class.
This example demonstrates how you can create a class that behaves like a sequence by implementing the required methods, without explicitly inheriting from the Sequence ABC. If you have any more questions or need further clarification, feel free to ask!